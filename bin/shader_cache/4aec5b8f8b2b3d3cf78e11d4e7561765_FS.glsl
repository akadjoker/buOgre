...
SAMPLER2D(shadow_map1, 1);
SAMPLER2D(shadow_map2, 2);
SAMPLER2D(gTextureSampler0, 0);
layout(location = 0) uniform	vec4	pssm_split_points0;
layout(location = 1) uniform	vec4	inverse_texture_size1;
layout(location = 2) uniform	vec4	inverse_texture_size2;
...
void main(void) {
	...
	SGX_ShadowPCF4(shadow_map1, vsTexcoord_0, inverse_texture_size1.xy, lShadowFactor[int(0.00000)]);
	SGX_ShadowPCF4(shadow_map2, vsTexcoord_1, inverse_texture_size2.xy, lShadowFactor[int(1.00000)]);
	gl_FragColor	=	derived_scene_colour;
	evaluateLight(vsTexcoord_2, vsTexcoord_3, light_position_view_space_array[int(0.00000)], light_attenuation_array[int(0.00000)], light_direction_view_space_array[int(0.00000)], spotlight_params_array[int(0.00000)], derived_light_diffuse_colour_array[int(0.00000)], gl_FragColor.xyz, lShadowFactor[int(0.00000)]);
	lColor_0	=	gl_FragColor;
	texel_0	=	texture2D(gTextureSampler0, vsTexcoord_4);
