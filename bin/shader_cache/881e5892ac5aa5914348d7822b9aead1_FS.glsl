...
layout(location = 4) uniform	vec4	light_position_view_space_array[1];
layout(location = 5) uniform	vec4	light_direction_view_space_array[1];
layout(location = 6) uniform	vec4	light_attenuation_array[1];
layout(location = 7) uniform	vec4	spotlight_params_array[1];
layout(location = 8) uniform	vec4	derived_light_diffuse_colour_array[1];
layout(location = 9) uniform	vec4	derived_light_specular_colour_array[1];
...
void main(void) {
	...
	vec2 local_vsTexcoord_0 = vsTexcoord_0;
	SGX_Generate_Parallax_Texcoord(gNormalMapSampler1, local_vsTexcoord_0, vsTexcoord_3, 0.0400000, TBN, local_vsTexcoord_0);
	local_vsTexcoord_0	=	local_vsTexcoord_0;
	SGX_FetchNormal(gNormalMapSampler1, local_vsTexcoord_0, lTexcoord_2);
	...
	texel_0	=	texture2D(gTextureSampler0, local_vsTexcoord_0);
