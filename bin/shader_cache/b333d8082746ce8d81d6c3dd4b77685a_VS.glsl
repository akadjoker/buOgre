...
layout(location = 48) uniform	mat3x4	bone_scale_shear_matrix_array_3x4[24];
layout(location = 120) uniform	mat4	worldviewproj_matrix;
layout(location = 124) uniform	mat3	normal_matrix;
layout(location = 127) uniform	mat4	worldview_matrix;
...
void main(void) {
	...
	blendBonesMat3x4(bone_scale_shear_matrix_array_3x4, blendIndices, blendWeights, blendS);
	vec4 local_vertex = vertex;
	FFP_Transform(blendS, local_vertex, local_vertex.xyz);
	...
	local_normal	=	mul(TempVal3x3, local_normal);
	local_normal	=	normalize(local_normal);
	SGX_CalculateBlendNormal(local_normal, blendDQ, local_normal);
	lColor_3	=	vec4(1.00000,1.00000,1.00000,1.00000);
	lColor_4	=	vec4(0.00000,0.00000,0.00000,0.00000);
